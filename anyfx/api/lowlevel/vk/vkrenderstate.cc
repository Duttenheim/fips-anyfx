...
*/
void
VkRenderState::SetupBlend(VkPipelineColorBlendStateCreateInfo* info, VkPipelineColorBlendAttachmentState* states)
{
    info->logicOp = vkLogicTable[this->renderSettings.logicOp];
    info->logicOpEnable = this->renderSettings.logicOpEnabled;

    unsigned i;
    for (i = 0; i < NumBlendSettings; i++)
    {
        states[i].blendEnable = this->renderSettings.blendEnabled[i];
        states[i].colorBlendOp = vkEquationTable[this->renderSettings.blendOps[i]];
        states[i].alphaBlendOp = vkEquationTable[this->renderSettings.alphaBlendOps[i]];
        states[i].srcColorBlendFactor = vkBlendTable[this->renderSettings.srcBlends[i]];
        states[i].dstColorBlendFactor = vkBlendTable[this->renderSettings.dstBlends[i]];
        states[i].srcAlphaBlendFactor = vkBlendTable[this->renderSettings.alphaSrcBlends[i]];
        states[i].dstAlphaBlendFactor = vkBlendTable[this->renderSettings.alphaDstBlends[i]];
