...
void LexerError(pANTLR3_BASE_RECOGNIZER recognizer, pANTLR3_UINT8* tokens)
{
	...
	if (state->token)
		file = (std::string*)state->token->custom;
	...
	{
		// print exception
		PrintLexerException(ex, file ? file->c_str() : nullptr);
...
void PrintLexerException(pANTLR3_EXCEPTION ex, const char* file)
{
...
#if _MSC_VER
			errorMessage = Format("%s(%d): error: Unexpected newline", fileString.c_str(), line);
#else
			errorMessage = Format("%s:%d: error: Unexpected newline\n", fileString.c_str(), line);
#endif
		}
		else
		{
	...
	else
	{
		if (character == 10)
		{
			//errorMessage = Format("Unexpected newline at: %d:%d in file %s\n", line, row, fileString.c_str());
#if _MSC_VER
			errorMessage = Format("(%d): error: Unexpected newline", line);
#else
			errorMessage = Format("(%d): error: Unexpected newline\n", line);
#endif
		}
		else
		{
			//errorMessage = Format("Lexer error: %s '%c' at %d:%d in file %s\n", exceptionAsString, character, line, row, fileString.c_str());
#if _MSC_VER
			errorMessage = Format("(%d): error: Unexpected character '%c', %s", line, character, exceptionAsString);
#else
			errorMessage = Format("(%d): error: Unexpected character '%c', %s\n", line, character, exceptionAsString);
#endif
		}
	}
